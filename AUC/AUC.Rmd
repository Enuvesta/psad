...
Сравним все значения критерия качества попарно с помощью критерия знаковых рангов Уилкоксона:
```{r}
p <- rep(0,6)
p[1] <- wilcox.test(AUCS[,1], AUCS[,2], paired=T, exact=T)$p.value
p[2] <- wilcox.test(AUCS[,1], AUCS[,3], paired=T, exact=T)$p.value
p[3] <- wilcox.test(AUCS[,1], AUCS[,4], paired=T, exact=T)$p.value
p[4] <- wilcox.test(AUCS[,2], AUCS[,3], paired=T, exact=T)$p.value
p[5] <- wilcox.test(AUCS[,2], AUCS[,4], paired=T, exact=T)$p.value
p[6] <- wilcox.test(AUCS[,3], AUCS[,4], paired=T, exact=T)$p.value
p
```
Без поправки на множественность отвергаются гипотезы о равенстве средних значений AUC у пар: 
...
p.adjust(p, "BY")
```
Любая поправка на множественность убивает значимость различий (на уровне 0.05).
